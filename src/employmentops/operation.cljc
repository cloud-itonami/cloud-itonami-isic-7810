(ns employmentops.operation
  "OperationActor -- one employment-agency operation = one supervised
  actor run, expressed as a langgraph-clj StateGraph. The advisor
  (EmploymentOps-LLM) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the Employment Agency Governor
  ...
  One graph run = one employment-agency operation (intake -> advise ->
  ...
  decision to a human operator (agency operator/placement
  coordinator). The approver resumes with `{:approval {:status
  :approved}}` (or :rejected). `:candidacy/match`/`:candidacy/place`
  ALWAYS reach this node when the governor is clean -- see
  `employmentops.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [employmentops.employmentopsllm :as employmentopsllm]
            [employmentops.governor :as governor]
...
(defn build
  ...
  `employmentops.store/Store`).
  opts:
    :advisor      -- an `employmentops.employmentopsllm/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (employmentopsllm/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; EmploymentOps-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (employmentopsllm/-advise advisor store request)]
            {:proposal p :audit [(employmentopsllm/trace request p)]})))

      ;; Employment Agency Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
